...
I will make two maps, one with a continuous scale and another with a thesholded KDE map so it might be easier to interpret.  
...
# --- CONFIG ---
GRID_NX, GRID_NY = 600, 600   # balanced resolution
BANDWIDTH = 650               # meters; adjust for sensitivity
...
# --- 1) Define region of interest (convex hull + buffer, clipped to boroughs) ---
pts_union = unary_union(gdf_multi_hosts.geometry)
roi = pts_union.convex_hull.buffer(BANDWIDTH * 2)
roi = roi.intersection(unary_union(boroughs.geometry))
...
ax.set_title("Continuous KDE of Multi-host Listings", fontsize=14)
ax.set_aspect("equal")
ax.axis("off")
plt.tight_layout()
plt.show()
...
# --- Normalize KDE surface to 0–1 range ---
z_norm = (z_masked - np.nanmin(z_masked)) / (np.nanmax(z_masked) - np.nanmin(z_masked))

# --- Define bin edges based on descending percentiles ---
# Bin 1: top 5%, Bin 2: 5–25%, Bin 3: 25–50%, Bin 4: 50–75%, Bin 5: 75–100%
percentiles = [95, 75, 50, 25, 0]
boundaries = np.nanpercentile(z_norm, percentiles)
boundaries = np.append(boundaries, 1.0)  # ensure upper bound is included
boundaries = sorted(boundaries)          # ascending order for BoundaryNorm

# --- Define colormap and normalization ---
cmap = colors.ListedColormap(["lightyellow", "gold", "orange", "red", "darkred"])
norm = colors.BoundaryNorm(boundaries, cmap.N)

# --- Compute bin centers for labeling ---
bin_centers = [(boundaries[i] + boundaries[i+1]) / 2 for i in range(len(boundaries) - 1)]
percent_labels = ["75–100%", "50–75%", "25–50%", "5–25%", "Top 5%"]

# --- Plot thresholded KDE ---
fig, ax = plt.subplots(figsize=(12, 12))
im = ax.imshow(
    z_norm,
    origin="upper",
...
# Colorbar with descriptive percentile labels
cbar = fig.colorbar(im, ax=ax, fraction=0.03, pad=0.04)
cbar.set_label("Density categories (percentile bins)", fontsize=12)
cbar.set_ticks(bin_centers)
cbar.set_ticklabels(percent_labels)

# Final styling
ax.set_title("Thresholded KDE of Multi-host Listings", fontsize=14)
ax.set_aspect("equal")
ax.axis("off")
plt.tight_layout()
plt.show()
